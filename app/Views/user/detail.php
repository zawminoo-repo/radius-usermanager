...
<div id="main" style="margin-left:200px">
	...
	<div class="w3-container w3-display-container">
		...
		<h3 class="h3" style="text-align:center;padding-top:20px"><strong>User Detail</strong></h3>
		
		<div class="w3-container w3-margin-bottom">
			<a href="<?= base_url(route_to('user_edit', $user->id)); ?>" class="w3-btn w3-orange"><i class="fa fa-edit"></i></a>
		</div>
		...
		<fieldset>
			...
			<div>
				<?php foreach($check_attributes as $check) : ?>
					<?= $check->username.' | '.$check->attribute.' '.$check->op; ?> <?= $check->attribute == 'SHA-Password' ? '*******' : $check->value; ?><br />
				<?php endforeach; ?>
			</div>
		</fieldset>
		</div>
		...
		<fieldset>
			...
			<div>
				<?php foreach($reply_attributes as $reply) : ?>
					<?= $reply->username.' | '.$reply->attribute.' '.$reply->op.' '.$reply->value; ?><br />
				<?php endforeach; ?>
			</div>
		</fieldset>
		</div>
