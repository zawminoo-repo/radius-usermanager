...
<div id="main" style="margin-left:200px">
	...
	<div class="w3-container w3-display-container">
		...
		<h3 class="h3" style="text-align:center;padding-top:20px"><strong>Edit Group</strong></h3>
		
		<div class="w3-container w3-margin-bottom">
		<fieldset>
			<form action="<?= base_url(route_to('group_edit', $group->id)); ?>" method="post">
			<?= csrf_field(); ?>
			...
			<div class="w3-row-padding" style="padding-top:10px">
				...
				<div class="w3-rest" style="margin-bottom:7px">
					<input class="w3-input w3-border" type="text" name="group_name" value="<?= $group->group_name; ?>">
				</div>
				<div class="w3-col m2" style="margin-bottom:7px">
					Description
				</div>
				<div class="w3-rest" style="margin-bottom:7px">
					<input class="w3-input w3-border" type="text" name="group_descript" value="<?= $group->group_descript; ?>">
				</div>
				<div class="w3-col">
		...
		<div class="w3-container w3-margin-bottom">
		<form action="<?= base_url(route_to('group_check_edit')); ?>" method="post">
		<?= csrf_field(); ?>
		<input type="hidden" name="_method" value="PUT" />
		...
		<fieldset>
			...
			<div>
				<div class="w3-row-padding" style="padding-top:10px;margin-bottom:7px">
					<div class="w3-col">
						<a href="<?= base_url(route_to('group_check_add', $group->id)); ?>" class="w3-btn w3-green"><i class="fa fa-plus"></i></a>
					</div>
				</div>
				<?php foreach($check_attributes as $check) : ?>
					<div class="w3-row-padding"  style="margin-bottom:7px">
						...
						<div class="w3-col m2">
							<a href="<?= base_url(route_to('group_check_delete')); ?>?id=<?= $check->id; ?>&group_id=<?= $group->id; ?>" onClick="return confirm('Are you sure want to delete?')"><i class="fa fa-trash" style="font-size:20px;color:red;padding-top:8px"></i></a>
						</div>
					</div>
				<?php endforeach; ?>
		...
		<form action="<?= base_url(route_to('group_reply_edit')); ?>" method="post">
		<?= csrf_field(); ?>
		<input type="hidden" name="_method" value="PUT" />
		<input type="hidden" name="group_id" value="<?= $group->id; ?>" />
		<fieldset>
			...
			<div>
				<div class="w3-row-padding" style="padding-top:10px;margin-bottom:7px">
					<div class="w3-col">
						<a href="<?= base_url(route_to('group_reply_add', $group->id)); ?>" class="w3-btn w3-green"><i class="fa fa-plus"></i></a>
					</div>
				</div>
				<?php foreach($reply_attributes as $reply) : ?>
					<div class="w3-row-padding" style="margin-bottom:7px">
						...
						<div class="w3-col m2">
							<a href="<?= base_url(route_to('group_reply_delete')); ?>?id=<?= $reply->id; ?>&group_id=<?= $group->id; ?>" onClick="return confirm('Are you sure want to delete?')"><i class="fa fa-trash" style="font-size:20px;color:red;padding-top:8px"></i></a>
						</div>
					</div>
				<?php endforeach; ?>
				<div class="w3-row-padding">
